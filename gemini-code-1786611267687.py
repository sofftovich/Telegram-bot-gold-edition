...
# Настройка логирования в stdout без задержек
logging.basicConfig(
    level=logging.INFO,
...
# Прокси (если заблокирован IP сервера, забей в .env PROXY_URL=http://user:pass@ip:port)
...
async def fetch_booru_posts(source, tags, limit=20):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(base_url, params=params, headers=headers, proxy=PROXY_URL, timeout=12) as resp:
                if resp.status == 200:
                    ...
                    if text_data.strip():
                        try:
                            ...
                            logger.error(f"❌ [НЕ XML - БЛОКИРОВКА CLOUDFLARE?] {source}: {xml_err}. Ответ: {text_data[:150]}")
...
async def handle_text_messages(message: Message):
    ...
    if text.startswith("/checkpost") or text.startswith("/testpost"):
        ...
        # Сначала пробуем Rule34, если там 0 — пробуем Gelbooru
        ...
        if not posts:
            await message.reply(f"❌ На Rule34 и Gelbooru вернулось 0 постов по тегу <code>{search_tag}</code>. Если IP сервера забанен Cloudflare — переключитесь на Gelbooru или добавьте PROXY_URL в .env.")
            return
