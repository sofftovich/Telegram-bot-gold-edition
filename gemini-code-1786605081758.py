...
def normalize_tags(raw_tags):
    """Превращает 'cat girl' в 'cat_girl', сохраняя разделители пробелов между тегами"""
...
async def start_add_label_wizard(message: Message):
    ...
    await message.reply(
        ...
        "Введите **название** лейбла (например: <code>Dross Art</code>):",
...
async def handle_wizard_callback(callback: CallbackQuery):
    ...
    if action.startswith("src_"):
        ...
        await callback.message.edit_text(
            ...
            "Пришлете **эмодзи** для маркировки (например, 🍆 или ID Custom Premium Emoji):"
        )
        await callback.answer()
    ...
    if uid in add_label_wizard:
        ...
        if step == "name":
            ...
            await message.reply(
                ...
                "Введите **теги** поиска через пробел.\n"
...
async def main():
    ...
    # Настройка разных подсказок выпадающего меню в ТГ для лички и групп
