...
async def init_db():
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            # Создаем таблицу и добавляем колонку queue, если ее не было
            await conn.execute("""
        ...
        if match:
            ...
            for cand in candidates:
                try:
                    ...
                    if diff > 720:  # Корректировка перехода через полночь
                        diff = 1440 - diff
                    if diff < min_diff:
                        min_diff = diff
                        best_match = cand
...
• /timezone - открыть интерактивное меню выбора региона
...
async def main():
    ...
    await bot.set_my_commands([
        ...
        BotCommand(command="days", description="📆 Разрешённые дни недели"),
