    ...
    if os.path.exists(STATE_FILE):
        try:
            with open(STATE_FILE, "r", encoding="utf-8") as f:
                ...
                POST_INTERVAL = state.get("post_interval", 7200)  # Дефолт 2 часа
...
"""
        ...
        try:
            ...
            old_interval = POST_INTERVAL
            POST_INTERVAL = new_interval
            
            # Если включено точное планирование и есть очередь, пересчитываем время следующего поста
            if EXACT_TIMING_ENABLED and load_queue():
                # Сбрасываем last_post_time чтобы бот пересчитал следующее точное время
                global last_post_time
                last_post_time = 0
            
            save_state()  # Сохраняем новый интервал
            
            # Показываем новые времена постинга если включено точное планирование
            interval_response = f"✅ Интервал изменен на {format_interval(POST_INTERVAL)}"
            
            if EXACT_TIMING_ENABLED:
                posting_times = calculate_exact_posting_times()
                if posting_times:
                    # Показываем следующее время постинга
                    ...
                    if next_exact_time:
                        ...
                        if next_exact_time <= now:
                            ...
                            seconds_until = int((next_exact_time - now).total_seconds())
                            next_time_text = f"через {format_interval(seconds_until)} (в {next_exact_time.strftime('%H:%M')})"
                        interval_response += f"\n\n🕐 Следующая публикация: {next_time_text}"
                    
                    # Ограничиваем показ до 10 времен
                    if len(posting_times) <= 10:
                        times_str = ", ".join([t.strftime('%H:%M') for t in posting_times])
                        interval_response += f"\n\n⏰ Новые времена постинга: {times_str}"
                    else:
                        first_10 = posting_times[:10]
                        times_str = ", ".join([t.strftime('%H:%M') for t in first_10])
                        remaining = len(posting_times) - 10
                        interval_response += f"\n\n⏰ Новые времена постинга: {times_str}\n<i>... и ещё {remaining} времен</i>"
            
            await message.reply(interval_response)
...
async def format_queue_response(media_text: str, added_count: int, queue: list, is_media_group: bool = False):
    ...
    # Рассчитываем время следующей публикации с учетом точного планирования
    now = get_czech_time()
    posting_allowed, reason = is_posting_allowed()
    delayed_ready = is_delayed_start_ready()
