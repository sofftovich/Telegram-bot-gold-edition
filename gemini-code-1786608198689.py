...
async def process_parsed_post(label, post_data, source):
    ...
    # Если пост уже выходил ранее — учитываем в статистике и пропускаем
    ...
    # Режим AUTO: сразу в очередь канала
    ...
    # Режим MANUAL: скачиваем байты и шлём в группу модерации
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(file_url, headers=headers, timeout=15) as resp:
                    if resp.status == 200:
                        ...
                        # Заносим в БД просмотренных только ПОСЛЕ успешной отправки
                        await mark_post_as_seen(source, post_id, file_md5)
    ...
    if is_group:
        if text == "/setmodgroup":
            ...
            await message.reply(
                ...
                "• /labels - интерактивное меню лейблов и удаление\n"
