...
# --- ЧИСТЫЙ ПАРСЕР ПОСТОВ С ФИЛЬТРАЦИЕЙ ИИ НА СТОРОНЕ БОТА ---

AI_TAGS = {"ai_generated", "created_by_ai", "novelai", "stable_diffusion", "midjourney", "ai_art"}

def is_ai_post(post_data):
    tags_str = post_data.get("tags", "").lower()
    post_tags = set(tags_str.split())
...
async def fetch_booru_posts(source, tags, limit=20):
    ...
    logger.info(f"🔍 [PARSER FETCH] {source} | Чистые теги: '{clean_tags}'")
                    ...
                    if text_data.strip():
                        try:
                            ...
                            for child in root.findall("post"):
                                p_dict = dict(child.attrib)
                                # Фильтрация ИИ на стороне бота, если ИИ запрещен
                                if not ALLOW_AI_POSTS and is_ai_post(p_dict):
                                    continue
                                posts.append(p_dict)
                            ...
                            logger.error(f"❌ [XML PARSE ERR] {source}: {xml_err}. Ответ: {text_data[:100]}")
        ...
        try:
            ...
            try:
                if file_id.startswith("http"):
                    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}
                    async with aiohttp.ClientSession() as session:
                        async with session.get(file_id, headers=headers, timeout=20) as resp:
                            if resp.status == 200:
...
async def handle_text_messages(message: Message):
    ...
    # --- КОМАНДА ОТПРАВКИ 1-го ПОСТА ПО ТЕГУ (ПРОВЕРКА В РЕАЛЬНОМ ВРЕМЕНИ) ---
    if text.startswith("/checkpost") or text.startswith("/testpost"):
        ...
        await message.reply(f"🔍 Запрашиваю Rule34 по тегу <code>{search_tag}</code>...")
        
        posts = await fetch_booru_posts("rule34", search_tag, limit=10)
        if not posts:
            await message.reply(f"❌ На Rule34 не найдено постов по тегу <code>{search_tag}</code>.")
            return

        first_post = posts[0]
        post_id = first_post.get("id")
        file_url = first_post.get("file_url")
        tags_attr = first_post.get("tags", "")

        source_link = f'<a href="https://rule34.xxx/index.php?page=post&s=view&id={post_id}">🟢 Rule34 (ID {post_id})</a>'
        caption = (
