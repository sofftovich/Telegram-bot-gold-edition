...
# --- ВСПОМОГАТЕЛЬНАЯ ОБРАБОТКА ТЕГОВ И ПАРСИНГ (XML + JSON) ---
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers, timeout=12) as resp:
                if resp.status == 200:
                    ...
                    if text_strip:
                        # 1. Проверяем XML-формат (если Rule34 прислал XML)
                        if text_strip.startswith("<") or "<?xml" in text_strip:
                            try:
                                ...
                                logger.error(f"❌ Ошибка парсинга XML от {source}: {xml_err}")
                                PARSER_STATS["network_errors"] += 1
                        # 2. Проверяем JSON-формат
                        else:
                            try:
                                ...
                                logger.error(f"❌ Ошибка парсинга JSON от {source}: {json_err}")
                                PARSER_STATS["network_errors"] += 1
                else:
                    logger.error(f"❌ {source} ответил статусом HTTP {resp.status}")
                    PARSER_STATS["network_errors"] += 1
    except Exception as e:
        PARSER_STATS["network_errors"] += 1
...
async def handle_wizard_callback(callback: CallbackQuery):
    ...
    if action.startswith("src_"):
        ...
        status_test = f"✅ Тестовый поиск успешен! Найдено постов на {sources[0]}: <b>{count_found}</b>" if count_found > 0 else "⚠️ Тестовый поиск выдал 0 постов. Проверьте правильность написания тегов."
