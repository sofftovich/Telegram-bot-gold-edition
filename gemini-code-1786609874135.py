...
def normalize_tags(raw_tags):
    ...
    if not ALLOW_AI_POSTS:
        if "-ai_generated" not in normalized:
            normalized.append("-ai_generated")
...
async def fetch_booru_posts(source, tags, limit=20):
    ...
    logger.info(f"🔍 [PARSER FETCH] {source} | Теги: '{clean_tags}'")

    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(base_url, params=params, headers=headers, timeout=12) as resp:
                logger.info(f"📡 [PARSER RESP] {source} Status: {resp.status} | URL: {resp.url}")
                if resp.status == 200:
                    ...
                    if text_data.strip():
                        try:
                            ...
                            logger.error(f"❌ [XML PARSE ERR] {source}: {xml_err}. Первые 100 симв: {text_data[:100]}")
...
async def handle_wizard_callback(callback: CallbackQuery):
    ...
    if action.startswith("src_"):
        ...
        ai_status_str = " (Фильтр Нейро-артов: ✅ ВКЛЮЧЕН)" if not ALLOW_AI_POSTS else " (Нейро-арты: ⚠️ РАЗРЕШЕНЫ)"
        status_test = f"✅ Найдено постов на {sources[0]}: <b>{count_found}</b>{ai_status_str}" if count_found > 0 else "⚠️ Тестовый поиск выдал 0 постов. Проверьте теги через /testsearch."
...
async def handle_text_messages(message: Message):
    ...
    # --- КОМАНДА ДИАГНОСТИКИ ПОИСКА В РЕАЛЬНОМ ВРЕМЕНИ ---
    if text.startswith("/testsearch"):
        parts = text.split(maxsplit=1)
        search_tag = parts[1].strip() if len(parts) > 1 else "femboy"
        await message.reply(f"🧪 Запускаю тестовую диагностику по тегу: <code>{search_tag}</code>...")
        
        res_posts = await fetch_booru_posts("rule34", search_tag, limit=10)
        if not res_posts:
            await message.reply("❌ Поиск вернул 0 постов. Проверьте логи Render.")
            return

        sample = res_posts[0]
        f_url = sample.get("file_url")
        p_id = sample.get("id")
        
        report = (
            f"🎉 <b>ТЕСТ УСПЕШЕН! Бот видит посты!</b>\n\n"
            f"📊 Найдено постов на Rule34: <b>{len(res_posts)} шт</b>\n"
            f"🆔 ID первого поста: <code>{p_id}</code>\n"
            f"🔗 URL: {f_url}\n\n"
            f"<i>Сейчас попробую прислать этот арт картинкой...</i>"
        )
        ...
        if f_url:
            try:
                headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}
                async with aiohttp.ClientSession() as session:
                    async with session.get(f_url, headers=headers, timeout=12) as r:
                        if r.status == 200:
                            img_bytes = await r.read()
                            p_file = BufferedInputFile(img_bytes, filename=f"test_{p_id}.jpg")
                            await message.reply_photo(photo=p_file, caption=f"🖼 Образец поста #{p_id}")
            except Exception as e:
                await message.reply(f"⚠️ Текст прошел, но отправить картинку не удалось: {e}")
        return
    ...
    if is_group:
        if text == "/setmodgroup":
            ...
            ai_str = "❌ Заблокированы (-ai_generated)" if not ALLOW_AI_POSTS else "✅ Разрешены"
            ...
            await message.reply(
                ...
                "• /testsearch [тег] - проверить живой поиск постов\n"
                "• /parser - детальная статистика сбора постов\n"
                "• /ai - вкл/выкл Нейро/ИИ арты (-ai_generated)\n"
                "• /clearseen - сброс базы дубликатов (пересбор заново)"
            )
            return

    # --- ЛИЧНЫЕ СООБЩЕНИЯ (ОБРАБОТКА КОМАНД ЛИЧКИ) ---
    if text == "/start":
...
async def main():
    ...
    await bot.set_my_commands([
        ...
        BotCommand(command="testsearch", description="🧪 Проверить живой поиск"),
