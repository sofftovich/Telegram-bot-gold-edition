...
def normalize_tags(raw_tags):
    """Нормализует теги и исправляет популярные опечатки (например, -antro -> -anthro)"""
...
async def fetch_booru_posts(source, tags, limit=20):
    ...
    # Формируем теги через символ '+' для гарантированной совместимости с API
                ...
                if resp.status == 200:
                    ...
                    if text_data.strip():
                        try:
                            ...
                            logger.error(f"❌ {source} вернул не JSON: {text_data[:100]}")
                else:
                    logger.error(f"❌ {source} ответил статусом {resp.status}")
    except Exception as e:
