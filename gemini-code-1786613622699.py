...
# API Ключи для Rule34 и Gelbooru
...
# --- ДВИЖОК ПАРСИНГА С ПОДДЕРЖКОЙ API-КЛЮЧЕЙ ---

async def fetch_booru_posts(source, tags, limit=20):
    """Запрашивает посты через API Rule34/Gelbooru с авторизацией по user_id и api_key"""
    posts = []
    ...
    if source == "rule34":
        ...
        if R34_USER_ID and R34_API_KEY:
            url += f"&user_id={R34_USER_ID}&api_key={R34_API_KEY}"
    ...
    try:
        # Микро-задержка от бана частых вызовов
