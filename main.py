...
# Настройка сквозного логирования (вывод в sys.stdout без задержек)
logging.basicConfig(
    level=logging.INFO,
...
# --- ВСПОМОГАТЕЛЬНАЯ ОБРАБОТКА ТЕГОВ И ГЛУБОКОЕ ЛОГИРОВАНИЕ ПАРСИНГА ---
...
async def fetch_booru_posts(source, tags, limit=20):
    ...
    encoded_tags = urllib.parse.quote_plus(clean_tags)

    if source == "rule34":
        url = f"https://api.rule34.xxx/index.php?page=dapi&s=post&q=index&json=1&tags={encoded_tags}&limit={limit}"
    elif source == "gelbooru":
        url = f"https://gelbooru.com/index.php?page=dapi&s=post&q=index&json=1&tags={encoded_tags}&limit={limit}"
    else:
        return posts

    logger.info(f"🔍 [PARSER REQUEST] Источник: {source} | Исходные теги: '{tags}' | Итоговые: '{clean_tags}' | URL: {url}")

    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers, timeout=12) as resp:
                logger.info(f"📡 [PARSER RESPONSE] {source} | HTTP Status: {resp.status}")
                if resp.status == 200:
                    text_data = await resp.text()
                    text_strip = text_data.strip()
                    
                    if not text_strip:
                        logger.warning(f"⚠️ [PARSER EMPTY] {source} вернул ПУСТОЕ тело ответа (0 байт)!")
                        return posts

                    # 1. Парсим XML
                    if text_strip.startswith("<") or "<?xml" in text_strip:
                        try:
                            root = ET.fromstring(text_strip)
                            for child in root.findall("post"):
                                posts.append(dict(child.attrib))
                            PARSER_STATS["total_scanned"] += len(posts)
                            logger.info(f"✅ [PARSER XML SUCCESS] {source} Найдено {len(posts)} постов через XML")
                        except Exception as xml_err:
                            logger.error(f"❌ [PARSER XML ERROR] Ошибка XML парсинга {source}: {xml_err}. Первые 200 симв: {text_strip[:200]}")
                    ...
                    # 2. Парсим JSON
                        ...
                        try:
                            ...
                            PARSER_STATS["total_scanned"] += len(posts)
                            logger.info(f"✅ [PARSER JSON SUCCESS] {source} Найдено {len(posts)} постов через JSON")
                        except Exception as json_err:
                            logger.error(f"❌ [PARSER JSON ERROR] Ошибка JSON парсинга {source}: {json_err}. Первые 200 симв: {text_strip[:200]}")
                            PARSER_STATS["network_errors"] += 1
                else:
                    logger.error(f"❌ [PARSER HTTP ERROR] {source} вернул статус {resp.status}")
                    PARSER_STATS["network_errors"] += 1
    except Exception as e:
        PARSER_STATS["network_errors"] += 1
        logger.error(f"❌ [PARSER NETWORK EXCEPTION] {source} | Ошибка: {e}")
...
async def process_parsed_post(label, post_data, source):
    ...
    if not file_url or not post_id:
        logger.warning(f"⚠️ Пост {source} без file_url или id, пропуск.")
        return
    ...
    if mode == "AUTO" and not is_suspicious:
        ...
        logger.info(f"⚡ [AUTO POST] Пост {source} #{post_id} закинут в очередь")
        return
    ...
    if MODERATION_CHAT_ID:
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(file_url, headers=headers, timeout=15) as resp:
                    if resp.status == 200:
                        ...
                        logger.info(f"📩 [MOD POST] Пост {source} #{post_id} успешно отправлен в группу модерации {MODERATION_CHAT_ID}")
                        ...
                        logger.error(f"❌ Ошибка скачивания фото {file_url} - HTTP {resp.status}")
        except Exception as e:
            PARSER_STATS["network_errors"] += 1
            logger.error(f"❌ Ошибка отправки фото {file_url} в группу: {e}")
    else:
        logger.warning(f"⚠️ MODERATION_CHAT_ID не установлен! Пост {source} #{post_id} пропущен.")

async def parser_loop():
    logger.info("🚀 [PARSER LOOP] Фоновый цикл сканирования запущен")
    while True:
        try:
...
async def handle_wizard_callback(callback: CallbackQuery):
    ...
    if action.startswith("src_"):
        ...
        logger.info(f"🧪 [TEST SEARCH] Тестовая проверка лейбла '{wdata['name']}' по тегам '{tags}' на источниках {sources}")
        ...
        status_test = f"✅ Тестовый поиск успешен! Найдено постов на {sources[0]}: <b>{count_found}</b>" if count_found > 0 else "⚠️ Тестовый поиск выдал 0 постов. Проверьте логи Render для подробностей."
...
async def handle_text_messages(message: Message):
    ...
    if is_group:
        if text == "/setmodgroup":
            ...
            await message.reply(
                f"🕵️‍♂️ <b>Полная статистика и статус Агрегатора:</b>\n\n"
                ...
                f"⏳ Время работы сессии: <b>{uptime_min} мин</b>\n\n"
                ...
                f"⚡ Автоматически отправлено в очередь: <b>{PARSER_STATS['auto_queued']}</b>\n"
                f"⚠️ Сбоев сети / API: <b>{PARSER_STATS['network_errors']}</b>"
            )
            return
            ...
            if db_pool:
                ...
                await message.reply("🧹 База просмотренных постов очищена! Парсер соберет все посты заново.")
            return
            ...
            await message.reply(
                ...
                "• /parser - детальная статистика сбора постов\n"
                "• /clearseen - сброс базы дубликатов (пересбор заново)"
            )
            return
    ...
    if text == "/start":
        ...
        await message.reply(
            ...
            f"⚡ Добавлено в авто-очередь: <b>{PARSER_STATS['auto_queued']}</b>\n"
