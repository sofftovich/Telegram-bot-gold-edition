...
# API Keys для Rule34 & Gelbooru (Авторизация)
...
# Чёрный список тегов для автоматического отсева ИИ-артов
AI_TAGS_BLACKLIST = {
    "ai_generated", "stable_diffusion", "novelai", "midjourney", 
    "dall-e", "anything_diffusion", "ai_art", "deepfake"
}

# Загрузка разрешённых пользователей
ALLOWED_USERS = []
for i in range(1, 10):
    user_id = os.getenv(f"ALLOWED_USER_{i}") or (os.getenv("ALLOWED_USER") if i == 1 else None)
    if user_id:
        try:
...
http_session: aiohttp.ClientSession = None  # Сессия aiohttp
...
async def fetch_booru_posts(source, tags, limit=20):
    """Запрашивает посты через JSON API Rule34/Gelbooru с передачей API Keys"""
    global http_session
    posts = []
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"}

    if source == "rule34":
        url = f"https://api.rule34.xxx/index.php?page=dapi&s=post&q=index&json=1&tags={tags}&limit={limit}"
        if RULE34_USER_ID and RULE34_API_KEY:
            url += f"&user_id={RULE34_USER_ID}&api_key={RULE34_API_KEY}"
    elif source == "gelbooru":
        url = f"https://gelbooru.com/index.php?page=dapi&s=post&q=index&json=1&tags={tags}&limit={limit}"
        if GELBOORU_USER_ID and GELBOORU_API_KEY:
            url += f"&user_id={GELBOORU_USER_ID}&api_key={GELBOORU_API_KEY}"
    else:
        return posts
    ...
    try:
        ...
        async with http_session.get(url, headers=headers, timeout=12) as resp:
            if resp.status == 200:
                ...
                logger.warning(f"⚠️ [HTTP 429] Лимит запросов к {source}. Убедитесь, что переданы корректные API Keys.")
    except Exception as e:
        logger.error(f"Ошибка запроса к {source} API ({tags}): {e}")
...
async def process_parsed_post(label, post_data, source):
    """Обрабатывает найденный пост: фильтрация ИИ, дедупликация, отправка в группу модерации или автопостинг"""
    ...
    # Фильтрация ИИ-артов (AI Blacklist Filter)
    if isinstance(post_tags, str):
        tags_set = set(post_tags.lower().split())
    elif isinstance(post_tags, list):
        tags_set = set(t.lower() for t in post_tags)
    else:
        tags_set = set()

    if not tags_set.isdisjoint(AI_TAGS_BLACKLIST):
        logger.info(f"🤖 [AI Filter] Арт {source} #{post_id} пропущен (содержит теги ИИ)")
        return
