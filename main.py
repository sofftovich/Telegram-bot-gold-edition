...
def is_posting_allowed():
    ...
    # Логи только для команд, не для автопостинга
    ...
    if WEEKDAYS_ENABLED and current_weekday not in ALLOWED_WEEKDAYS:
        # print(f"❌ Постинг запрещён: день недели не разрешён")
    ...
    if TIME_WINDOW_ENABLED:
        ...
        if START_TIME <= END_TIME:
            ...
            # print(f"🔍 Проверка обычного окна: {START_TIME.strftime('%H:%M')} <= {current_time.strftime('%H:%M')} <= {END_TIME.strftime('%H:%M')} = {time_in_window}")
            if not time_in_window:
                # print(f"❌ Постинг запрещён: вне временного окна")
            ...
            # print(f"🔍 Проверка ночного окна: {current_time.strftime('%H:%M')} >= {START_TIME.strftime('%H:%M')} OR {current_time.strftime('%H:%M')} <= {END_TIME.strftime('%H:%M')} = {time_in_window}")
            if not time_in_window:
                # print(f"❌ Постинг запрещён: вне временного окна")
                return False, "вне временного окна"

    # print(f"✅ Постинг разрешён")
...
def create_web_app():
    ...
    app.router.add_post('/', health_check)  # Для POST запросов тоже
