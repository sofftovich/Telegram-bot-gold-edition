...
# Настройка логирования в stdout Render
logging.basicConfig(
    level=logging.INFO,
...
PARSER_SPEED = 15               
PARSER_LIMIT = 20               
ALLOW_AI_POSTS = False          # Разрешать ли Нейро/ИИ арты (по умолчанию выключено!)
LABELS = []                     
...
# --- ЧИСТЫЙ XML ПАРСЕР ПОСТОВ ---

def normalize_tags(raw_tags):
    """Нормализует теги, убирает опечатки и фильтрует ИИ"""
    ...
    # Если нейропосты ЗАПРЕЩЕНЫ, подклеиваем фильтр исключения AI
...
async def fetch_booru_posts(source, tags, limit=20):
    ...
    # Используем ЧИСТЫЙ XML DAPI (без json=1), так как он работает со 100% стабильностью
    if source == "rule34":
        url = f"https://api.rule34.xxx/index.php?page=dapi&s=post&q=index&tags={encoded_tags}&limit={limit}"
    elif source == "gelbooru":
        url = f"https://gelbooru.com/index.php?page=dapi&s=post&q=index&tags={encoded_tags}&limit={limit}"
    else:
        return posts

    logger.info(f"🔍 [PARSER FETCH] {source} | Итоговые теги: '{clean_tags}' | URL: {url}")
                ...
                if resp.status == 200:
                    ...
                    if text_data.strip():
                        try:
                            ...
                            logger.info(f"🎉 [PARSER SUCCESS] {source}: Успешно распознано {len(posts)} постов!")
                        except Exception as xml_err:
                            logger.error(f"❌ [XML PARSE ERR] {source}: {xml_err}. Ответ: {text_data[:150]}")
...
async def handle_wizard_callback(callback: CallbackQuery):
    ...
    if action.startswith("src_"):
        ...
        status_test = f"✅ Найдено постов на {sources[0]}: <b>{count_found}</b>{ai_status_str}" if count_found > 0 else "⚠️ Тестовый поиск выдал 0 постов. Проверьте правильность написания тегов."
...
async def handle_text_messages(message: Message):
    ...
    # --- КОМАНДА ВМЕСТЕНИЯ НЕЙРО-ПОСТОВ ---
    if text == "/ai":
